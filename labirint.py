...
class GameSprite(sprite.Sprite):
    ...
    def __init__(self, player_image, player_x, player_y, size_x, size_y):
        ...
        # каждый српайт должен хранить свойство image - изображение
        self.image = transform.scale(image.load(player_image), (size_x, size_y))
        # каждй спрайт должен хранить свойство rect - прямоугольник, в который он вписан
...
class Player(GameSprite):
    ...
    def update(self):
        ...
        # если зашли за стенку, то встанем в плотную к стене
        platforms_touched = sprite.spritecollide(self, barriers, False)
        if self.x_speed > 0: # идём на право, правый край персонажа - вплотную к левому краю стены
            for p in platforms_touched:
                self.rect.right = min(self.rect.right, p.rect.left) #если коснулись сразу нескольких, то правый край - минимаьный из возможных
        elif self.x_speed < 0: #идём на лево, ставим левый край персонажа вплотную к правому краю стены
            for p in platforms_touched:
...
class Enemy(GameSprite):
    ...
    #движение врага
    def update(self):
        if self.rect.x <= 420: #w2.wall_x + w1.wall_width
            self.side = 'right'
...
class Bullet(GameSprite):
    ...
    def update(self):
        self.rect.x += self.speed
        # исчезнет, если дойдёт до края экрана
...
back = (119, 210, 223)#задаём цвет согласно уветовой схеме RGB
#создаём группу для стен
barriers = sprite.Group()
#создаём группу для пуль
bullets = sprite.Group()
#создаём группу для монстров
...
monster = GameSprite('cyborg.png', win_width - 80, 100, 80, 80)
...
while run:
    ...
    if not finish:
        ...
        packman.update()
        bullets.update()
        #обновляем и в новом местоположении при каждой итеграции цикла
        packman.reset()
        #рисуем объекты
        # w1.reset()
        # w2.reset()
        ...
        #Проверка столконовения героя с врагом и стенами
